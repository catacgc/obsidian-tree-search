...
export class NotesGraph {
	...
	private createRefsNodes(obsidianLinkReference: ObsidianRef[], textNode: TextNode): ParsedNode[] {
	...
	private createNodeFromText(page: DvPage, item: DvList): TextNode | PageNode | HeaderNode {
		...
		const obsidianLinkReference = this.getObsidianLinkReference(tokens);
	...
	private getRefsFromString(pageReference: string): ObsidianRef[] {
		const tokens = [...parseMarkdown(pageReference, {})];
		return this.getObsidianLinkReference(tokens)
	}
	...
	private getObsidianLinkReference(tokens: Token[]): ObsidianRef[] {
		...
		for (const token of obisidianLinks) {
			...
			const fileName = this.resolveReference(headerSplit[0])
			const header = headerSplit[1]

			refs.push({source: actualRef, pageTarget: fileName, headerName: header, alias: alias})
		}

		return refs
	}

	private createHeader(pageReference: ObsidianRef, location: BaseNode['location']): HeaderNode | null {
	 ...
	 */
	private createVirtualPage(pageReference: ObsidianRef, location: BaseNode['location']): PageNode | HeaderNode {
		const page: PageNode = {
			nodeType: "page",
			isReference: true,
