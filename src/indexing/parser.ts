...
function inlineHttpLinkPlugin(md: MarkdownIt) {
    // Transform plain URLs into markdown links before parsing
    md.core.ruler.before('normalize', 'transform-urls', state => {
        const urlRegex = /(^|\s)(https?:\/\/[^\s\])}>,]+)/gm;
        state.src = state.src.replace(urlRegex, (match, space, url) => {
            // Don't transform if it's already part of a markdown link [...](...) or <...>
            const prevChar = space || '';
            if (prevChar.endsWith('[') || prevChar.endsWith('<')) {
                return match;
            }
...
function _parseMarkdown(markdownText: string, env: any) {
	// Initialize markdown-it with the custom plugins
	const md = new MarkdownIt()
		.use(obsidianLinkPlugin)
		.use(inlineHttpLinkPlugin);
...
function _parseTokens(tokens: Token[]): ParsedTextToken[] {
    ...
    if (token.type == "http_link") {
        const href = token.attrs?.[0]?.[1] || token.content
        return [
            {tokenType: "link", href: href, content: token.content},
    ...
    if (token.type == "image") {
        return [
            {tokenType: "image", src: token.attrs?.[0]?.[1] || "", alt: token.content || token.attrs?.[1]?.[1] || ""},
            ..._parseTokens(tokens.slice(1))
        ]
    }

    if (!token.type.includes("_close")) console.log("tokens not rendered: ", token)
