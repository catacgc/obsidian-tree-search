...
export const SearchTreeList = (props: SearchTreeListProps) => {
    ...
    const [userExpanded, setUserExpanded] = useState(props.level < props.minExpand)
    const expanded = props.level < props.minExpand
    useEffect(() => setUserExpanded(false), [props.minExpand])
    ...
    return (
        <>
            ...
            {(expanded || userExpanded) && props.node.children.map((child) => (
                <SearchTreeList
                    node={child}
                    level={props.level + 1}
